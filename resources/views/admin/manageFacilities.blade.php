<x-general-layout>
                        ...
                        <div class="col-sm-8">
                            <div class="text-sm-end mt-3">
                                <a type="button" href="#" class="btn btn-primary mb-2 me-1"
                                    data-bs-toggle="modal" data-bs-target="#staticBackdrop">Add Facility</a>
                            </div>
                        </div><!-- end col-->
                    </div>
                </div>
                            ...
                            <table id="datableButtons" class="table table-striped mb-0 w-100 ">
                                ...
                                <tbody>
                                    @foreach ($facilities as $key => $facility)
                                        <tr>
                                            <td>{{ $key + 1 }}</td>
                                            <td>{{ $facility->facility_name }}{{ $facility->parent != null ? ' (' . $facility->parent->facility_name . ')' : '' }}
                                            </td>
                                            ...
                                            <td class="table-action">
                                                ...
                                                <a class="action-icon"> <i class="mdi mdi-pencil" data-bs-toggle="modal"
                                                        data-bs-target="#editFacility{{ $facility->id }}"></i></a>
    ...
    @foreach ($facilities as $key => $facility)
        <div class="modal fade" id="editFacility{{ $facility->id }}" data-bs-backdrop="static" data-bs-keyboard="false"
                        ...
                        <form method="POST" action="{{ route('facilities.update', [$facility->id]) }}">
                            ...
                            <div class="row">
                                <div class="mb-3 col-md-6">
                                    <label for="parent_facility{{ $facility->id }}" class="form-label">Parent
                                        facility</label>
                                    <select class="form-select" id="parent_facility{{ $facility->id }}"
                                        name="parent_id">
                                        @if ($facility->parent != null)
                                            <option selected value="{{ $facility->parent_id }}">
                                                {{ $facility->parent->facility_name }}</option>
                                        @endif
                                        <option value="">None</option>
                                        @foreach ($allfacilities as $key => $facility2)
                                            <option value='{{ $facility2->id }}'>{{ $facility2->facility_name }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="mb-3 col-md-6">
                                    <label for="facilityName{{ $facility->id }}" class="form-label">
                                        Facility Name</label>
                                    <input type="text" id="facilityName{{ $facility->id }}" class="form-control"
                                        name="facility_name" value="{{ $facility->facility_name }}">
                                </div> <!-- end col -->

                                <div class="mb-3 col-md-6">
                                    <label for="facility_type{{ $facility->id }}" class="form-label">Type</label>
                                    <select class="form-select" id="facility_type{{ $facility->id }}"
                                        name="facility_type">
                                        <option selected value="{{ $facility->facility_type }}">
                                            {{ $facility->facility_type }}</option>
                                ...
                                <div class="mb-3 col-md-6">
                                    ...
                                    <input type="text" id="requestername" class="form-control" name="requester_name"
                                        value="{{ $facility->requester_name }}">
                                </div>
                                <div class="mb-3 col-md-4">
                                    ...
                                    <input type="text" id="requestercontact" class="form-control"
                                        name="requester_contact" value="{{ $facility->requester_contact }}">
                                </div>
                                <div class="mb-3 col-md-5">
                                    <label for="requester_contact" class="form-label">
                                        Requester Email</label>
                                    <input type="email" id="requester_email" class="form-control"
                                        name="requester_email" value="{{ $facility->requester_email }}">
                                </div>
